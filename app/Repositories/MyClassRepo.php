...
class MyClassRepo
{
    ...
    public function getClassesByExamID($id)
    {
        return Mark::where('exam_id', $id)->orderBy('name', 'asc')->get();
    }
    ...
    public function getClassesByExam(Request $request)
    {
        $exam_id = $request->exam_id;
        $classes = Mark::where('exam_id', $exam_id)
            ->distinct()
            ->pluck('my_class_id');

        $class_details = MyClass::whereIn('id', $classes)->get();
