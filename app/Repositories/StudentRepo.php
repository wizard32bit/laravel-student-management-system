...
class StudentRepo {
    ...
    {
        return StudentRecord::where(['my_class_id' => $class_id])->with(['my_class', 'user'])->get()->sortBy('user.name');
    }
    ...
    {
        return StudentRecord::where('section_id', $sec_id)->with(['user', 'my_class'])->get();
    }
    ...
    {
        return StudentRecord::where($data)->with('user');
    }
