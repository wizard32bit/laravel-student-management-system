...
class Qs
{
    ...
    {
        return self::getSetting('system_title') ?: 'MLIU';
    }
    ...
    public static function getDefaultUserImage()
    {
        if (Auth::check()) {
            ...
            */
            ...
            $image = array_key_exists($user_type,$user_images) ? 'profile/'.$user_images[$user_type] : 'user.png';

            return asset('global_assets/images/' . $image);
        }
        return asset('global_assets/images/user.png');
    }


    public static function getImagePathOnSeeding($user_type)
    {
        $user_images = [
            'super_admin' => 'super_admin.png',
            'student' => 'student.png',
        ...
        // Check if the user type exists as a key
        $image = array_key_exists($user_type, $user_images) ? 'profile/'.$user_images[$user_type] : 'user.png';
        return asset('global_assets/images/' . $image);
    }
 ...
 <div class="alert alert-success alert-bordered">
    ...
    {
        $data = ['name', 'email', 'phone', 'dob', 'gender', 'address', 'bg_id', 'nal_id', 'state_id', 'lga_id'];
