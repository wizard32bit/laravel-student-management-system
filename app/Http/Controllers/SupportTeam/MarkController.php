...
class MarkController extends Controller
{
    ...
    public function show($student_id, $year)
    {
        ...
        $exr = $this->exam->getRecord($wh);// ExamRecord::where($data)->get();
        $d['exam_records'] = $exr;

        $d['exams'] = $this->exam->getExam(['year' => $year]);// Exam::where($data)->get();
        $d['sr'] = $this->student->getRecord(['user_id' => $student_id])->first();// this->activeStudents()->where($data)->with('user');
        
        $mc = $this->my_class->getMC(['id' => $exr->first()->my_class_id])->first();// $this->my_class->MyClass::where(['id' => $exr->first()->my_class_id])->with('section');
        $d['my_class'] = $mc; 
        ...
        //return Qs::goWithDanger('marks.bulk', json_encode($d));
...
/*367*/
    ...
    {
        $d['my_classes'] = $this->my_class->all();// from 'my_classes' table
        $d['exams'] = $this->exam->getExam(['year' => $this->year]);// from 'exams' table
        $d['selected'] = FALSE;
        ...
        if($class_id && $section_id && $exam_id){
            ...
            $sub_ids = $this->mark->getSubjectIDs($wh);// get subjects from 'marks' table
            $st_ids = $this->mark->getStudentIDs($wh); // get student from 'marks' table 
            
            if(count($sub_ids) < 1 OR count($st_ids) < 1) {
                return Qs::goWithDanger('marks.tabulation', __('msg.rnf'));
            }
    ...
    {
        if(!($req->exam_id && $req->my_class_id && $req->section_id)){
            return Qs::goWithDanger('marks.tabulation', __('msg.rnf'));
        }
