...
class PinController extends Controller
{
    ...
    {
        // Check if generating the new pins will exceed the limit
        $currentValidPins = $this->pin->countValid();
        $maxPins = 500;

        if ($currentValidPins + $req->pin_count > $maxPins) {
            return redirect()->route('pins.create')->with('flash_danger', 'You can only have up to 500 valid pins.');
        }
        ...
        return redirect()->route('pins.index')->with('flash_success', 'Pins generated successfully.');
    }
